...
class ChainHashTableMapTest : public ::testing::Test {
protected:
    ChainHashTableMap<int, std::string> emptyMap;
    ChainHashTableMap<int, std::string> map;
...
TEST_F(ChainHashTableMapTest, DefaultConstructedIsEmpty) {
    EXPECT_TRUE(emptyMap.empty());
    EXPECT_EQ(emptyMap.size(), 0);
}

TEST_F(ChainHashTableMapTest, InsertAndSize) {
    ChainHashTableMap<int, std::string> m;
    EXPECT_TRUE(m.empty());
    EXPECT_EQ(m.size(), 0);
...
TEST_F(ChainHashTableMapTest, FindExistingKey) {
    auto it = map.find(2);
...
TEST_F(ChainHashTableMapTest, FindNonExistingKey) {
    auto it = map.find(100);
    EXPECT_EQ(it, map.end());
}

TEST_F(ChainHashTableMapTest, SubscriptAccessExisting) {
    EXPECT_EQ(map[1], "one");
    EXPECT_EQ(map[3], "three");
    EXPECT_EQ(map.size(), 4);
}

TEST_F(ChainHashTableMapTest, SubscriptInsertNew) {
...
TEST_F(ChainHashTableMapTest, SubscriptModifyExisting) {
    map[2] = "two modified";
    EXPECT_EQ(map[2], "two modified");
}

TEST_F(ChainHashTableMapTest, EraseByKeyExisting) {
...
TEST_F(ChainHashTableMapTest, EraseByKeyNonExisting) {
    size_t erased = map.erase(100);
    EXPECT_EQ(erased, 0);
    EXPECT_EQ(map.size(), 4);
}

TEST_F(ChainHashTableMapTest, EraseByIterator) {
    auto it = map.find(3);
    ASSERT_NE(it, map.end());
...
TEST_F(ChainHashTableMapTest, IterationCount) {
...
TEST_F(ChainHashTableMapTest, ModifyViaIterator) {
    auto it = map.find(1);
    ASSERT_NE(it, map.end());
    it->second = "modified";
...
TEST_F(ChainHashTableMapTest, Rehash) {
    ChainHashTableMap<int, int> map(2);
    const int N = 100;
    for (int i = 0; i < N; ++i) {
        map.insert({i, i * 10});
    }
...
TEST(ChainHashTableMapStringTest, StringKeys) {
    ChainHashTableMap<std::string, int> m;
    m.insert({"apple", 10});
    m.insert({"banana", 20});
    m.insert({"cherry", 30});
...
TEST(ChainHashTableMapLargeTest, ManyElements) {
    ChainHashTableMap<int, int> m;
    const int N = 1000;
...
TEST(ChainHashTableMapEraseTest, EraseAndReinsert) {
    ChainHashTableMap<int, int> m;
    m.insert({1, 10});
    m.erase(1);
    EXPECT_EQ(m.size(), 0);
...
TEST(ChainHashTableMapStressTest, ManyInsertErase) {
    using Map = ChainHashTableMap<int, int>;
    Map map;
    ...
    std::cout << "[Stress] ChainHashTableMap ManyInsertErase (" << OPS << " ops) done in "
              << elapsed.count() << " sec\n";
...
TEST(ChainHashTableMapStressTest, ManyFinds) {
    using Map = ChainHashTableMap<int, int>;
    Map map;
    ...
    auto end = std::chrono::steady_clock::now();
    std::chrono::duration<double> elapsed = end - start;
    std::cout << "[Stress] ChainHashTableMap ManyFinds (" << SEARCH_OPS << " finds) done in "
              << elapsed.count() << " sec\n";
    ...
    elapsed = end - start;
    std::cout << "[Stress] ChainHashTableMap ManyOperatorBrackets (" << SEARCH_OPS << " ops) done in "
              << elapsed.count() << " sec\n";
}