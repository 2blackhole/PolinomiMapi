...
class AVLTreeMapTest : public ::testing::Test {
protected:
    AVLTreeMap<int, std::string> emptyMap;
...
TEST_F(AVLTreeMapTest, DefaultConstructedIsEmpty) {
    EXPECT_TRUE(emptyMap.empty());
    EXPECT_EQ(emptyMap.size(), 0);
}

TEST_F(AVLTreeMapTest, InsertAndSize) {
    AVLTreeMap<int, std::string> m;
    EXPECT_TRUE(m.empty());
    EXPECT_EQ(m.size(), 0);
...
TEST_F(AVLTreeMapTest, FindExistingKey) {
    auto it = map.find(2);
...
TEST_F(AVLTreeMapTest, FindNonExistingKey) {
    auto it = map.find(100);
    EXPECT_EQ(it, map.end());
}

TEST_F(AVLTreeMapTest, SubscriptAccessExisting) {
    EXPECT_EQ(map[1], "one");
    EXPECT_EQ(map[3], "three");
    EXPECT_EQ(map.size(), 4);
}

TEST_F(AVLTreeMapTest, SubscriptInsertNew) {
...
TEST_F(AVLTreeMapTest, SubscriptModifyExisting) {
    map[2] = "two modified";
    EXPECT_EQ(map[2], "two modified");
}

TEST_F(AVLTreeMapTest, EraseByKeyExisting) {
...
TEST_F(AVLTreeMapTest, EraseByKeyNonExisting) {
    size_t erased = map.erase(100);
    EXPECT_EQ(erased, 0);
    EXPECT_EQ(map.size(), 4);
}

TEST_F(AVLTreeMapTest, EraseByIterator) {
    auto it = map.find(3);
    ASSERT_NE(it, map.end());
...
TEST_F(AVLTreeMapTest, EraseByIteratorEnd) {
    auto it = map.find(4);
    auto next = map.erase(it);
...
TEST_F(AVLTreeMapTest, IterationOrder) {
...
TEST_F(AVLTreeMapTest, InsertMaintainsOrder) {
    map.insert({0, "zero"});
...
TEST_F(AVLTreeMapTest, EraseMaintainsOrder) {
    map.erase(2);
...
TEST_F(AVLTreeMapTest, ModifyViaIterator) {
    auto it = map.find(1);
    ASSERT_NE(it, map.end());
    it->second = "modified";
...
TEST(AVLTreeMapStringTest, StringKeys) {
    AVLTreeMap<std::string, int> m;
    m.insert({"apple", 10});
    m.insert({"banana", 20});
    m.insert({"cherry", 30});
...
TEST(AVLTreeMapLargeTest, ManyElements) {
...
TEST(AVLTreeMapEraseTest, EraseAndReinsert) {
    AVLTreeMap<int, int> m;
    m.insert({1, 10});
    m.erase(1);
    EXPECT_EQ(m.size(), 0);
...
TEST(AVLTreeMapStressTest, ManyInsertErase) {
    using Map = AVLTreeMap<int, int>;
    Map map;
...
TEST(AVLTreeMapStressTest, ManyFinds) {
    using Map = AVLTreeMap<int, int>;
    Map map;

    const int N = 1000000;
    const int SEARCH_OPS = 10000000;
